...
{ 
  ...
  public static function validarInicioSesion($email, $password)
  {
    ...
    for ($i=0; $i < count(json_decode($contenidoArchivo)); $i++)
    {
      if($usuarios[$i]["correo"] == $email && $usuarios[$i]["contrasena"] == $password)
      {
        ...
        $respuesta["codigoUsuario"] = $usuarios[$i]["codigoUsuario"];
  ...
  public static function editarUsuario($codigoUsuario,$nombre,$apellido,$contrasena)
  {
    $contenidoArchivo = file_get_contents("../data/usuarios.json");
    $usuarios = json_decode($contenidoArchivo, true);
    for ($i=0; $i < count(json_decode($contenidoArchivo)); $i++) { //decodificanco el archivo en un arreglo y contando sus elementos
      if($usuarios[$i]["codigoUsuario"] == $codigoUsuario) { //comparando usuarios si tiene el mismo codigo que queremos editar
        // Cambiando la informacion del usuario por los nuevos datos
  ...
  public static function borrarUsuario($codigoUsuario)
  {
    # code...
  }
}